...
import { startExploration, state } from "@/components/state"
  ...
  return (
    <Panel icon={RocketLaunchIcon} title="Send robot">
      <form className="grid grid-cols-4 gap-5" onSubmit={onSubmit}>
        <InputField label="X" name="x" type="number" defaultValue={0} min={0} max={width - 1} required />
        <InputField label="Y" name="y" type="number" defaultValue={0} min={0} max={height - 1} required />
